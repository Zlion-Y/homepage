...
本仓库自带一份 **serverless 版的音源解析**（`api/` + `lib/`），跟主页一起部署在 Vercel：

- 前端调**同源**的 `/api/url`（`config.js` 里 `musicProxy` 留空即为同源），
  所以不需要单独域名、证书、CORS 白名单，也不用维护另一台服务器；
- 服务端用 Node 原生 `vm` 跑洛雪音源脚本（脚本本来就是 JS，连垫片都不用），
  多音源按成绩分波对冲、赢家一出即掐断其余、直链探活、连续失败熔断——逻辑与原项目一致；
- 直链缓存放在 CDN 边缘（`s-maxage=900`），**命中缓存的请求根本不进函数**，不消耗调用次数。

**加音源**：把 `.js` 放进 `sources/` 一起部署，或配 `SOURCE_URLS` 环境变量指向在线脚本
（不重新部署也能换源）。详见 [`sources/README.md`](sources/README.md)。

部署后打开 `https://你的域名/api/health` 可以看到装上了哪些音源、各自的平台与失败原因。

> 说明：函数跑在境外（默认 `hkg1` 香港），实测对国内音乐接口的成功率只比国内直连低约 4 个百分点
> （55.1% vs 59.0%），但机房 IP 被风控的概率高于家宽，属灰区用法，建议只自用、别公开分发。
