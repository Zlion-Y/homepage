...
export function createHost({ code, file, initTimeout = 5000, fetchImpl = globalThis.fetch }) {
  ...
  function lxRequest(url, options, cb) {
    ...
    // 归属当前 invocation（见 invoke 的说明）：所属调用一取消/超时，在飞的请求跟着掐
    const invSignal = lx.invocationSignal
  ...
  const lx = {
    version: '2.6.0',
    env: 'desktop',
    // 内部字段（非洛雪契约，脚本不应依赖）：当前 invocation 的取消信号。
    // invoke() 每次调用换一个，lx.request 发请求时挂上去；调用结束/取消/超时
    // 会 abort 它，把这次调用还挂在网络上的请求一并掐断
    invocationSignal: null,
  ...
  return {
     ...
     * 实现方式：invoke 每次换一个新的 invocation 信号挂到 lx 上，lx.request
     * 发请求时把自己的 AbortController 挂到该信号下（见 lxRequest）。
     */
