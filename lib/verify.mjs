...
export async function verifyDirectLink(url, { timeout = 900, fetchImpl = globalThis.fetch } = {}) {
  ...
  try {
    ...
    if (c1.ok) return { status: c1.status, contentType: c1.type }

    // 第一条不是活链 → 等另一条确认（HEAD 403 / 不被支持 都可能被 GET 翻案）
    const c2 = classify(await (first.tag === 'head' ? get : head))
    if (c2.ok) return { status: c2.status, contentType: c2.type }
