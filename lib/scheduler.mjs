 ...
 */
import { abortErr } from './lx-host.mjs'
  ...
  async function resolveWithHedge({ source, action, info, hosts, signal }) {
    ...
    try {
      ...
      const fire = (h, restAfter) => {
        ...
        Promise.resolve()
          ...
          .finally(() => {
            ...
            // restAfter 是该候选所属波次派发完后的未派发快照——不读实时游标 i，
            // 未来任何人改动波次推进方式都不会让这个判断漏算
            if (!result && fallback) {
              const betterLeft =
                [...inflight].some((f) => (qualityOf(f) ?? 1) > fallbackQ + 0.05) ||
                restAfter.some((hh) => (qualityOf(hh.file) ?? 1) > fallbackQ + 0.05)
      ...
      while (i < ranked.length) {
        ...
        // 该波派发完仍未派发的候选快照：fire 的 finally 据此判断"更好的候选还会不会来"
        const restAfter = ranked.slice(i + width)
        ranked.slice(i, i + width).forEach((h) => fire(h, restAfter))
        i += width
