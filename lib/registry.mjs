 ...
 *     改脚本 = 重新部署。要"不重新部署也能更新音源"，就把脚本放远端，并配置
 *     vercel.json 的 crons 定时触发 refresh()（当前模板未配置 crons，手动调
 *     GET /api/health?refresh=1 即可）。
 */
...
// 环境变量里的路径不强求尾斜杠，这里统一补齐 —— 否则 SOURCES_DIR + f 会拼出
// ".../sourcex.js" 这类路径，readdirSync 直接抛错、全源加载失败且线上零征兆
const SOURCES_DIR = (
